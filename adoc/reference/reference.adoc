...
|Mandatory, allowed values: `{secret-types}`
...
.Data example
operation::save-transfer[snippets='request-body']

.Result example
operation::save-transfer[snippets='response-body']
...
Arkane also offers whitelabel wallets. link:whitelabel.html[Go to documentation]
...
.URL
https://connect-staging.arkane.network/signature/execute

.[[generic-sign-message]] Data parameters
operation::build-sign-generic-message[snippets='request-fields']

.Data example
operation::build-sign-generic-message[snippets='request-body']

.Result parameters
operation::sign-and-execute-generic-message[snippets='response-fields']

.Result example
operation::sign-and-execute-generic-message[snippets='response-body']
...
This can be used to transfer native tokens from a wallet to another wallet.
To start a transfer you need to:
...
====
[[build-value]]
^[1]^ As *value*, we expect the "*decimal value*".
If a token has 18 decimals and you want to transfer 1 token, provide the value 1. Arkane will translate this to the correct non decimal value (1 * 10e18).
...
[[token-transfer]]
=== Token Transfer link:buildingblocks.html#_arkane_connect[[.bb-connect]#Arkane Connect#]
...
[[gas-transfer]]
=== Gas Transfer link:buildingblocks.html#_arkane_connect[[.bb-connect]#Arkane Connect#]
...
[[nft-transfer]]
=== NFT Transfer link:buildingblocks.html#_arkane_connect[[.bb-connect]#Arkane Connect#]
...
=== Contract execution link:buildingblocks.html#_arkane_connect[[.bb-connect]#Arkane Connect#]
Generic endpoint to execute a function on a smart contract (write) on any (supported) blockchain.
As a result, a new transaction will be submitted to the network containing the smart contract execution.

.URL
https://connect-staging.arkane.network/transaction/execute

.[[generic-contract-execution-data-parameters]] Data parameters
operation::build-generic-contract-execution[snippets='request-fields']
...
.Data example
This example will execute an ERC20 transfer using the direct execution of the smart contract function "transfer".
The inputs for this function are: destination (address) and amount (uint265)
operation::build-generic-contract-execution[snippets='request-body']
...
[[contract-execution-supported-chains]]
==== Supported chains
===== Ethereum
Supported types:
[source]
...
|outputs
|It is possible to add additional asset transfers together with a contract call by attaching additional outputs. This is a JSON object containing "to", "amount" and "assetId" with json types
respectively string, number and string. If "to" is a script hash, it will be translated to a valid NEO address
|JSON
|
{
"to": "AKJrLM5Q...",
...
== [[native-transactions]] Native Transactions
...
==== [[sign-raw-aeternity-transaction]] Sign raw AEternity transaction link:buildingblocks.html#_arkane_connect[[.bb-connect]#Arkane Connect#]
To sign a raw AEternity transaction, use the connect building block with:

.URL
https://connect-staging.arkane.network/transaction/sign/aeternity_raw


.Data parameters
operation::sign-aeternity-raw[snippets='request-fields']

.Data example
operation::sign-aeternity-raw[snippets='request-body']

.Result parameters
operation::sign-aeternity-raw[snippets='response-fields']

.Result example
operation::sign-aeternity-raw[snippets='response-body']
...
==== [[execute-matic-transaction]] Execute Matic transaction link:buildingblocks.html#_arkane_connect[[.bb-connect]#Arkane Connect#]

To execute a Matic transaction, use the connect building block with:

.URL
https://connect-staging.arkane.network/transaction/execute/matic_transaction


.Data parameters
operation::execute-matic-transaction[snippets='request-fields']

[NOTE]
====
[[matic-network]]
^[1]^ The node to which the transaction will be send can be set manually.
This can be done using the `network` parameter. +
This feature enables you to submit a transaction to any mainnet node of your choosing, a testnet node or a different (private) network altogether.
====

.Data example
operation::execute-matic-transaction[snippets='request-body']

.Result parameters
operation::execute-matic-transaction[snippets='response-fields']

.Result example
operation::execute-matic-transaction[snippets='response-body']

==== Execute Matic ERC20 transfer link:buildingblocks.html#_arkane_connect[[.bb-connect]#Arkane Connect#]

To execute a ERC20 transaction transfer, use the connect building block with:

.URL
https://connect-staging.arkane.network/transaction/execute/matic_erc20_transaction


.Data parameters
operation::execute-matic-erc20-transaction[snippets='request-fields']
...
.Data example
operation::execute-matic-erc20-transaction[snippets='request-body']

.Result parameters
operation::execute-matic-erc20-transaction[snippets='response-fields']

.Result example
operation::execute-matic-erc20-transaction[snippets='response-body']

==== Execute Matic ERC721 transfer link:buildingblocks.html#_arkane_connect[[.bb-connect]#Arkane Connect#]

To execute an ERC721 transaction transfer, use the connect building block with:

.URL
https://connect-staging.arkane.network/transaction/execute/matic_erc721_transaction


.Data parameters
operation::execute-matic-erc721-transaction[snippets='request-fields']

[NOTE]
====
[[matic-erc721-network]]
^[1]^ The node to which the transaction will be send can be set manually.
This can be done using the `network` parameter. +
This feature enables you to submit a transaction to any mainnet node of your choosing, a testnet node or a different (private) network altogether.
====
...
.Data example
operation::execute-matic-erc721-transaction[snippets='request-body']

.Result parameters
operation::execute-matic-erc721-transaction[snippets='response-fields']

.Result example
operation::execute-matic-erc721-transaction[snippets='response-body']

==== Sign link:buildingblocks.html#_arkane_connect[[.bb-connect]#Arkane Connect#]

Signs arbitrary data.
This data is before UTF-8 HEX decoded and enveloped as followed:
...
.URL
https://connect-staging.arkane.network/transaction/sign/matic_raw

.Data parameters
operation::sign-matic-hex-message[snippets='request-fields']

.Data example
operation::sign-matic-hex-message[snippets='request-body']

.Result parameters
operation::sign-matic-hex-message[snippets='response-fields']

.Result example
operation::sign-matic-hex-message[snippets='response-body']
...
|transaction.submit.matic-error
|Something went wrong while trying to submit the matic transaction
