...
| `windowMode` | false (default = 'POPUP') | <<WindowMode>> The sign method you want to use, possible values are 'POPUP', 'POPUP_WITH_OVERLAY' or 'REDIRECT' | 'REDIRECT'
...
{
    POPUP = 'POPUP',
    POPUP_WITH_OVERLAY = 'POPUP_WITH_OVERLAY',
    REDIRECT = 'REDIRECT'
}
----
|===
...
| `POPUP_WITH_OVERLAY` | *Recommended*: Same as `POPUP` but also add an overlay to the current window to block other actions.
...
* For windowMode = 'POPUP' and 'POPUP_WITH_OVERLAY', when the user finishes the popup closes and the promise resolves. No extra options are required.
...
[source,javascript,options="nowrap"]
----
arkaneConnect.createSigner(signUsing?: WindowMode): Signer
----
