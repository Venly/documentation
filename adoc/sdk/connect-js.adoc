...
| `closePopup`  | false (default = true) | If true, the popup closes when the action is finished. But is used internally when multiple actions that need a popup are called in sequence. When setting it to false the next action will be loaded in the same popup without first closing it.
|===
...
arkaneConnect.flows.getAccount('ETHEREUM').then((account: Account) => {
    if(account.isAuthenticated) {
        ...
        console.log('name', auth.tokenParsed.name);
    }
}).catch((e) => {
